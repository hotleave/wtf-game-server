        ...
        try {
            switch (task.getTaskType()) {
                ...
                case MESSAGE:
                    ...
                    JsonNode m = Util.parseJson(task.getMessage());
                    String type = m.get("k").asText();
                    JsonNode map = m.get("v");
                    ...
                    switch (type) {
                        case "login":
                            ...
                            if (!Server.version.equals(map.get("ver").asText())) {
                            ...
                            if (mode == 0) {
                                for (int i = 1; i <= Channel.MAX_ROOM_COUNT; i++) {
                                    if (Channel.getChannel(i).getPlayers().size() < 16) {
                                        ...
                                        player.setName(map.get("name").asText());
